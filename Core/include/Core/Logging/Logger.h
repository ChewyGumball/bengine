...
static void Log(const LogCategory& category, LogLevel level, const std::string_view message, Args&&... args) {
...
template <typename... Args>
static inline void Trace(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Trace, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Debug(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Debug, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Info(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Info, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Warning(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Warning, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Error(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Error, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Critical(const LogCategory& category, const std::string_view message, Args&&... args) {
    Log(category, LogLevel::Critical, message, std::forward<Args>(args)...);
}

template <typename... Args>
static inline void Always(const LogCategory& category, const std::string& message, Args&&... args) {
